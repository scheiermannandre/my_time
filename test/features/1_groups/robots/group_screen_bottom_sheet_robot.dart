...
class GroupScreenBottomSheetRobot {
  ...
  Future<void> expectIsOpen() async {
    ...
    final termsOfUseListTile = find.byKey(GroupsScreenController.termsOfUseKey);
  ...
  Future<void> _openAndClosePolicyDialog(Finder widget) async {
    ...
    expect(find.byType(PolicyDialog), findsOneWidget);
    final closeBtn = find.byKey(PolicyDialog.closePolicyDialogBtnKey);
    expect(closeBtn, findsOneWidget);
    await TestRobot.clickWidget(tester, closeBtn);
    expect(find.byType(PolicyDialog), findsNothing);
  }
}
